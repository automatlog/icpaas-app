          ...
          senders.map((s, i) => {
            ...
            return (
              <View key={s.senderId || i} className={`rounded-[20px] p-4 mb-3 ${softBg}`} style={{ borderWidth: 1, borderColor: c.bgInput }}>
                ...
                <InfoRow
                  label="Sender ID" value={s.senderId}
                  onCopy={() => copy(s.senderId, 'Sender ID')}
                  inputBg={inputBg} textInk={textInk} textMuted={textMuted} muted={c.muted}
                />
                <InfoRow
                  label="Entity ID (PE ID)" value={s.entityId}
                  onCopy={() => copy(s.entityId, 'Entity ID')}
                  inputBg={inputBg} textInk={textInk} textMuted={textMuted} muted={c.muted}
                />
                <InfoRow
                  label="Hash chain ID" value={s.hashChainId}
                  onCopy={() => copy(s.hashChainId, 'Hash chain')}
                  inputBg={inputBg} textInk={textInk} textMuted={textMuted} muted={c.muted}
                />
              </View>
            );
